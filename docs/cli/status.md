...
- The `alternate_extra_wall` print feature.
- The camera-fullscreen GUI feature.

Both remain fully reachable in git history via the `archive/v2.7-cli`
tag (`6cb539d2f`), which is `201` commits past `v02.07.00.55`
(`git rev-list --count v02.07.00.55..archive/v2.7-cli`).

### Triage outcomes (expectation changes + upstream cause)

- **Zero source-code changes needed in `src/cli` or `tests/cli`.**
  `load_bbs_3mf` / `store_bbs_3mf` / `StoreParams`,
...
- E2E smoke (manual, via `bambu-cli.exe` directly): `project init`
  ...
  identical counts; all invocations exit 0.
...
`[x]` — **signed off 2026-08-24.** `bcli_smoke.3mf`, produced by the
`v02.08.02.61`-built CLI, was opened in Bambu Studio 02.08.02.61 (host
version matches the port's base tag exactly) and **sliced
successfully**.

The slice matters more than the open: loading only proves the 3MF
parses, whereas slicing drives config resolution, per-object overrides
and geometry through the full BS pipeline. A CLI-authored project that
slices clean is the strongest evidence available that the 2.08 port
produces genuinely valid output, not merely loadable output.

Artifact: `C:\Users\ildar\Documents\bambu-cli-signoff\bcli_smoke.3mf`,
generated by the deployed `bambu-cli-bin\bambu-cli.exe` running steps
2–12 of `docs/cli/manual-test.md` (16 CLI operations, 0 failures). Final
state as the recipe specifies: 2 plates (`Plate 01 test`,
`FinalLayout`), 2 objects (`000_01_test_cylinder`,
`000_01_test_bambu_cube`), both extruder 4, 4 filament slots.

**Scope of this sign-off:** the Step 15 final-state check. The
per-step intermediate visuals (four objects on Plate-2 before the
removals, the 0.5 mm project line width, the 0.4 mm per-object
override) are NOT covered — those states no longer exist in the final
artifact, and verifying them needs a snapshot per checkpoint. This is
the first GUI sign-off in the port lineage since M12 (2026-06-01).
