...
TEST_CASE("split_object_to_parts: multi-volume object -> invalid_argument", "[unit][split]") {
    ...
    // Second split: 2 volumes -> must fail (mesh-state error, exit 7)
    REQUIRE_THROWS_AS(bambu_cli::split_object_to_parts(s, "twin"), bambu_cli::InvalidStateError);
}

TEST_CASE("split_object_to_parts: single-component mesh -> InvalidStateError", "[unit][split]") {
    ...
    REQUIRE_THROWS_AS(bambu_cli::split_object_to_parts(s, "box"), bambu_cli::InvalidStateError);
}
